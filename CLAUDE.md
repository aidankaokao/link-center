...
**路由** — 使用 `react-router-dom` v6 的 `BrowserRouter`。`App.tsx` 內部的 `AppRoutes` 元件定義所有路由，頁面對應如下：`/` → Home、`/link` → Link、`/chat` → Chat、`/tts` → Tts、`/celebrity` → Celebrity、`/learner` → Learner、`/ebook` → Ebook、`/paper` → PaperDoc、`/user-manage` → UserManage、`/admin-manage` → AdminManage。頁面切換使用 `useNavigate`，子頁面返回首頁透過 `onBack` prop 傳入 `() => navigate('/')`。
...
| 檔案 | 功能 |
|---|---|
| `Home.tsx` | 卡片式導覽頁。`NAV_ITEMS` 陣列驅動卡片格線（`auto-fit minmax(230px,270px)` + `justify-content:center`，`max-width:1200px`）：帶有 `page` 屬性（路由路徑字串）的項目執行內部導航（`useNavigate`），帶有 `href` 的項目開啟外部連結，帶有 `disabled` 的項目顯示為 Coming Soon，帶有 `forRole: 'user' \| 'admin'` 的項目依登入身份過濾（guest 不顯示）。目前卡片：01 頁面連結（hardcoded）、02 LLM 問答、03 民國人物傳、04 學習者、05 電子書問答、06 文獻探索、07 帳戶管理（user only）、08 系統管理（admin only）。（TTS 卡片已暫時移除，`Tts.tsx` 保留備用）右上角有 `UserCircleIcon` 按鈕（guest=灰、user=藍、admin=綠），點擊展開懸浮框：未登入時顯示登入／註冊 tab 表單（httpOnly cookie session）；已登入時顯示帳號名稱、角色 badge、前往管理頁按鈕、登出。主題切換按鈕向左移至 `right: 76px` 以為 user icon 留位。 |
